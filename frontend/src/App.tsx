...
const MODEL_PROVIDER_PRESETS: ModelProviderPreset[] = [
  ...
  },
  {
    ...
    defaultModel: "",
    defaultCapabilities: ["llm"],
    description: "自定义服务需要填写 Base URL，并选择 OpenAI 兼容或 Anthropic Messages。",
  },
]
...
function protocolLabel(protocol: CompanyModel["protocol"]): string {
  return protocol === "anthropic_messages" ? "Anthropic Messages" : "OpenAI 兼容"
}
...
function CompanySettingsPage({ auth, capabilities }: { auth: AuthPayload; capabilities: Capabilities | null }) {
  ...
  const modelRows = [
    ...
    {
      type: "视觉模型",
      model: capabilities?.providers.model || "默认视觉模型",
  ...
  const displayedModels = models.length > 0
    ? models.map((model) => ({
      ...
      model: model.model,
      provider: `${modelProviderPreset(model.provider).label} / ${protocolLabel(model.protocol)}`,
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[300px_minmax(0,1fr)] gap-4 overflow-auto p-4">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <section className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-neutral-600">官方供应商固定协议和端点；只有自定义模型需要选择兼容协议。</p>
              </div>
              <div className="flex items-center gap-2">
                <button
              ...
              {modelForm.provider === "custom" ? (
                ...
                <Field label="官方协议 / 端点">
                  <div className="flex h-9 items-center rounded-md border border-neutral-200 bg-neutral-50 px-3 text-xs text-neutral-600">
                    <span className="mr-2 rounded bg-white px-1.5 py-0.5 text-[11px] text-cyan-700">{protocolLabel(selectedProviderPreset.protocol)}</span>
          ...
          <section className="grid grid-cols-3 gap-4">
            ...
            <PolicyCard icon={<Check className="h-4 w-4" />} title="默认策略" text="新知识库只使用 llm_wiki 形态，模型和搜索策略继承默认值。" />
          </section>
        </div>
      </div>
    </section>
