...
summary: "以 nashsu/llm_wiki 为核心底座，设计企业级知识网络 KN（Knowledge Network）的完整系统架构。覆盖从员工上传文档到 Agent 探索知识的全链路：多入口上传、服务化改造、多租户、图谱引擎、混合检索、MCP Server。"
series: ["KN 企业知识网络"]
series_order: 2
---
...
## 〇、企业核心诉求：从"能上传"到"能用上"

在进入架构设计之前，先明确企业知识网络要解决的**真实痛点链**。这不是技术选型的学术讨论，而是每天在发生的事：

### 痛点链

```
员工上传了一份 PDF ──→ 躺在某个文件夹里
        ↓
有人 3 个月后需要它 ──→ 忘了文件名，搜不到
        ↓
终于找到了 ──→ 但不知道这份文档和公司已有知识的关系
        ↓
读了文档 ──→ 发现结论和另一份文档矛盾，但不知道找谁确认
        ↓
最终 ──→ 知识有，但不可用
```
...
| # | 诉求 | 从用户视角 | 从系统视角 |
|---|------|-----------|-----------|
| 1 | **方便上传** | 拖拽/粘贴/链接，任何格式都行，不挑 | 多入口 + 多格式解析 + 异步处理 |
| 2 | **自动理解** | 上传后不用管，系统自动读懂 | 两步 CoT 编译 + 实体/关系抽取 + 矛盾检测 |
| 3 | **能找到** | 模糊记得一点内容就能搜到 | 关键词 + 语义向量 + 图谱扩展三阶段检索 |
| 4 | **能看懂** | 不只是搜到片段，要看到知识全景 | 编译后的 wiki 页面 + 交叉引用 + 社区聚类 |
| 5 | **不冲突** | 两篇矛盾的内容要被标记出来 | 编译时矛盾检测 + lint 持续审查 |
| 6 | **人 + Agent 共用** | 人用 Web UI，Agent 用 MCP | 双通道统一 API |
| 7 | **持续可信** | 知识不过时，来源可追溯 | 声明级出处 + 过时检测 + review 审计 |
...
**KN（Knowledge Network）** 是基于 nashsu/llm_wiki 改造的企业级知识网络平台。核心理念继承自 Karpathy LLM Wiki 范式——**增量构建持久化知识库，知识随时间复利增长** [1]，并在此基础上叠加企业级能力。
...
*文档版本 v0.2 | 2026-05-25 | 基于 nashsu/llm_wiki v0.4.13 架构分析*

## 参考来源

1. Karpathy, A. "LLM Wiki." Gist, April 2026. <https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f>
2. nashsu/llm_wiki. GitHub. <https://github.com/nashsu/llm_wiki>
3. axoviq-ai/Synthadoc. GitHub. <https://github.com/axoviq-ai/synthadoc> — 企业级 LLM Wiki 参考设计
4. atomicstrata/llm-wiki-compiler. GitHub. <https://github.com/atomicstrata/llm-wiki-compiler> — MCP Server 参考设计
5. Cliff-AI-Lab/Wiki-Map. GitHub. <https://github.com/Cliff-AI-Lab/wikimap> — 双引擎架构参考
